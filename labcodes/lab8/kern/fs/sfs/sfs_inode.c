...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2016011395 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	 ...
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op	
	*/
    #define ERROR_CHECK if(ret != 0) goto out;
    //not aligned at begin
    if(offset % SFS_BLKSIZE > 0){
        ret = sfs_bmap_load_nolock(sfs,sin,blkno,&ino);
        ERROR_CHECK;
        //if total small than a block
        if(nblks == 0){
            alen = endpos - offset;
            ret = sfs_buf_op(sfs,buf,endpos - offset,ino,offset%SFS_BLKSIZE);
            ERROR_CHECK;
            goto out;
        }else{
            alen = SFS_BLKSIZE - offset%SFS_BLKSIZE;
            ret = sfs_buf_op(sfs,buf,SFS_BLKSIZE - (offset%SFS_BLKSIZE),ino,offset%SFS_BLKSIZE);
            ERROR_CHECK;
            blkno += 1;
            nblks -= 1;
            buf += SFS_BLKSIZE - offset%SFS_BLKSIZE;
            if(nblks == 0) goto out;
        }
    }
    // aligned blks
